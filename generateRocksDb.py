...
def popenAndCall(bench, workload, inits, opers, msize, threads, opt, dist, serial):
    name = runName(bench, workload, inits, opers, msize, threads, opt)
    try:
        cmd = './ycsbc -db {0} -P workloads/{1}.spec -threads {2} -recordcount {3} -operationcount {4} -munkBytesCapacity {5} {6}' \
             .format(bench, workload, threads, inits, opers, msize, opt)
        print('\t\tStarting: ' + cmd)
        ...
        onError(ex, name)
    dbPath = "rocksdbSavedDbs/" + dist+"-" + serial
    if not os.path.exists(dbPath):
        os.makedirs(dbPath)
    subprocess.call("tar -C "+ rocksDataDir+"/ -cvzf " + dbPath + "/savedDb.tar.gz .", shell=True)
    deleteFiles()
    return
...
commonOps = ''
piwi_commonOps = ''
rocks_options = ['-rocksdb_cachesize 1073741824 -use_existing_db 0']#, '-rocksdb_cachesize 1073741824 -statistics yes -stats_dump_period_sec 600']
...
munk_sizes = ['196608'] #5MB 
workloads = ['init_zip']
initsNum = ['0']
opersNum = ['60000000']
piwi_options = ['-maxMunks 400 -writeBufBytesCapacity 524288 -initChunks 5000']
                ...
                for msize in munk_sizes:
                    for threads in threadsNum:
                        ...
                        for opt in opts:
                            ...
                            popenAndCall(bench, workload, inits, opers, msize, threads, opt, "zipfian", "random")
                ...
                for msize in munk_sizes:
                    for threads in threadsNum:
                        ...
                        for opt in opts:
                            opt = opt + ' ' + commonOps
                            print('\tinits: ' + inits + ', operations: ' + opers + ', munk_bytes_capacity: ' + msize + ', threads: ' + threads + ', ops: ' + opt)
                            popenAndCall(bench, workload, inits, opers, msize, threads, opt, "flurry", "random")

############: ############################
################ serial initialization of dbse  #############
                ...
                for msize in munk_sizes:
                    for threads in threadsNum:
                        ...
                        for opt in opts:
                            opt = opt + ' ' + commonOps
                            print('\tinits: ' + inits + ', operations: ' + opers + ', munk_bytes_capacity: ' + msize + ', threads: ' + threads + ', ops: ' + opt)
                            popenAndCall(bench, workload, inits, opers, msize, threads, opt, "zipfian", "serial")
                ...
                for msize in munk_sizes:
                    for threads in threadsNum:
                        ...
                        for opt in opts:
                            opt = opt + ' ' + commonOps
                            print('\tinits: ' + inits + ', operations: ' + opers + ', munk_bytes_capacity: ' + msize + ', threads: ' + threads + ', ops: ' + opt)
                            popenAndCall(bench, workload, inits, opers, msize, threads, opt, "flurry", "serial")
...
#initsNum = ['0']
#opersNum = ['60000000']
#piwi_options = ['-maxMunks 4000 -writeBufBytesCapacity 524288 -initChunks 50000']
...
#munk_sizes = ['2522'] #64KB 
#workloads = ['init_zip']
#initsNum = ['0']
#opersNum = ['60000000']
#piwi_options = ['-maxMunks 32768 -writeBufBytesCapacity 524288 -initChunks 500000']
