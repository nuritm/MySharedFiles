...
def popenAndCall(bench, workload, inits, opers, msize, threads, opt, dist, serial):
    ...
    try:
        ...
        print('About to copy DB from '+dbPath + ' to ' + rocksdbDataDir)
        if not os.path.exists(rocksdbDataDir):
              os.makedirs(rocksdbDataDir)
        subprocess.call("tar -C "+rocksdbDataDir+" -xzf "+dbPath+"/savedb.tar.gz", shell=True)
...
piwi_commonOps = ''
keyrange = '15000000'
munk_sizes = ['196608'] #5MB, 1MB, 500KB, 64KB 


workloads = ['puts_gets_zip', 'puts_only_zip', 'gets_only_zip']
opersNum = ['60000000'] 
initsNum = ['0'] 
rocks_options = ['-rocksdb_cachesize 1073741824 -use_existing_db 1', '-rocksdb_cachesize 1073741824 -statistics yes -stats_dump_period_sec 600 -use_existing_db 1']


piwi_options = ['-maxMunks 400 -writeBufBytesCapacity 524288 -initChunks 5000 -use_existing_db true'] 
